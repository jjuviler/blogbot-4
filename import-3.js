...
function handleFile(file) {
    JSZip.loadAsync(file)
        ...
        .catch(err => {
            console.error('[blogbot] handleFile pipeline error:', err);
...
function processImagesAndSave(originalZip, imgDetails) {
    ...
    Promise.all(imagePromises).then(() => {
        ...
        console.error('[blogbot] processImagesAndSave error:', err);
        alert("WebP image conversion failed. Please check your Google Doc for any broken images and try again. If you are still having issues, reach out to Jamie.");
    });
}
    ...
    return file.async('blob').then(blob => {
        ...
        if (detail) {
            ...
            if (fileExtension === 'gif') {
                ...
                console.warn('[blogbot] Skipping TIFF file (not supported by browser canvas):', fileName);
            } else {
                return convertImageToWebP(blob).then(webpBlob => {
                    newZip.file(newFileName, webpBlob, { binary: true });
                }).catch(err => {
                    console.error('[blogbot] convertImageToWebP failed for', fileName, ':', err);
                    alert("WebP image conversion failed. Please check your Google Doc for any broken images and try again. If you are still having issues, reach out to Jamie.");
                });
            }
        } else {
            console.warn('[blogbot] No matching detail for', fileName);
        }
    }).catch(err => {
        console.error('[blogbot] processImageFile error for', file.name, ':', err);
        alert("WebP image conversion failed. Please check your Google Doc for any broken images and try again. If you are still having issues, reach out to Jamie.");
    });
}
